...
object Sort {
    ...
    fun nameSort(string: String, items: List<Int>): List<Int> {
        return items.filter { ChatColor.stripColor(Man10Commerce.getDisplayName(ItemDataOld.itemDictionary[ItemDataOld.orderMap[it]!!.itemID]!!))!!
            .toLowerCase().contains(string.toLowerCase()) }
    }

    fun materialSort(material: Material, items: List<Int>): List<Int> {
        return items.filter { ItemDataOld.itemDictionary[ItemDataOld.orderMap[it]!!.itemID]!!.type == material }
    }

    fun sellerSort(seller : String, items: List<Int>): List<Int> {
        val uuid = Bukkit.getOfflinePlayer(seller).uniqueId
        return items.filter {uuid == ItemDataOld.orderMap[it]?.seller }
    }

    fun enchantSort(enchantment: Enchantment, level: Int, items: List<Int>): List<Int> {
        return items.filter {
            val meta = ItemDataOld.itemDictionary[ItemDataOld.orderMap[it]!!.itemID]!!.itemMeta
